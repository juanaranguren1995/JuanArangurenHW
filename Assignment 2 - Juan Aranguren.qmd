...
shootings_month <- mass_shootings %>%
  group_by(month) %>% 
  summarize(count = n()) %>% 
  mutate(month = shootings_per_month$Month, shootings_per_month$n, .desc = TRUE)

#Then we generate the bar chart
bar_chart <- ggplot(shootings_month, aes(x = month, y = count, fill = month)) +
  geom_bar(stat = "identity") +
...
argentina_plots <- create_country_plots("Argentina")
argentina_plots
```
